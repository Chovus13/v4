...
def filter_walls(orderbook, current_price, threshold=0.001):  # Možeš testirati i sa 0.005
    ...
    bids = np.array(orderbook['bids'], dtype=float)  # Konverzija u float
    asks = np.array(orderbook['asks'], dtype=float)
    bid_volumes = bids[:, 1]
    ask_volumes = asks[:, 1]
    total_bid_volume = sum(bid_volumes)
    total_ask_volume = sum(ask_volumes)
    logger.debug(f"Bids: {len(bids)}, Asks: {len(asks)}, Total bid volume: {total_bid_volume:.2f}, Total ask volume: {total_ask_volume:.2f}")
    ...
    for i in range(len(bids) - 10):
        ...
        logger.debug(f"Support cluster {i}: volume={cluster_volume:.2f}, price_spread={price_spread:.6f}")
    ...
    for i in range(len(asks) - 10):
        ...
        cluster_volume = sum(cluster_volumes)
        price_spread = max(cluster_prices) - min(cluster_prices)
        logger.debug(f"Resistance cluster {i}: volume={cluster_volume:.2f}, price_spread={price_spread:.6f}")
        if (price_spread <= WALL_RANGE_SPREAD and cluster_volume >= MIN_WALL_VOLUME):
...
def detect_trend(orderbook, current_price):
    ...
    logger.debug(f"Buy pressure: {buy_pressure:.2f}, Sell pressure: {sell_pressure:.2f}")
